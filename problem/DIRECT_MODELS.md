# Earlier history-only forecasting models

The current default is now one weather-aware CatBoost per turbine; see
[WEATHER_CONTEXT_MODELS.md](WEATHER_CONTEXT_MODELS.md). This document describes the retained
history-only bundle. To run it, explicitly pass
`--model-dir problem/models_direct_missing` to `problem/predict.py`.

The history-only inference bundle is `models_direct_missing/`. Each turbine has
